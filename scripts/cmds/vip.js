...
async function loadIgnoredUIDs() {
  try {
    const response = await axios.get("https://raw.githubusercontent.com/Aiden-Josiah/josiah/refs/heads/main/skApiUrl.json");
...
async function fetchMessages(language) {
  const url = language === "bn"
    ? "https://raw.githubusercontent.com/Aiden-Josiah/josiah/refs/heads/main/sk-bn.json"
    : "https://raw.githubusercontent.com/Aiden-Josiah/josiah/refs/heads/main/sk-en.json";
  try {
...
module.exports.onChat = async function ({ api, event }) {
  ...
  if (messages.length > 0) {
    ...
    await api.sendMessage(
      `🎉 Hi ${userNames[senderID] || "User"}  ${randomMessage} 🎉`,
      threadID,
      (error, messageInfo) => {
        if (error) {
          console.error("Error sending message:", error);
        } else {
          global.GoatBot.onChat.set(messageInfo.messageID, {
            commandName: this.config.name,
            type: "reply",
            messageID: messageInfo.messageID,
            author: userNames[senderID] || "User",
            link: randomMessage
          });
        }
      },
